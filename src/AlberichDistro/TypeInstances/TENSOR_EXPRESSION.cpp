...
{
    // LOG << "SINGLE::operator< aufgerufen " << lhs.toString(1) << " " << rhs.toString(1) << endln;
    // LOG << std::flush;
...
bool operator<(const std::pair<TensorExpression, TensorExpression>& lhs, const std::pair<TensorExpression, TensorExpression>& rhs) {
    ...
    // LOG << "PAIR::operator< aufgerufen" << " " << lhs.first.toString() << " " << rhs.first.toString() << endln;
    // LOG << std::flush;
...
bool TensorExpression::assembleSubstitutionMap(const TensorExpression& tmplExpr, const TensorExpression& expr, substitutionMap& subsMap, bool disableLog){
    ...
    // LOG << tmplExpr.toString() << " <> " << expr.toString() << " repl? " << tmplIsTemplatedNode << endln;
                ...
                *this = v;
                ...
                if(g_compareTemplateDependencies){
                    ...
                    if(k.containsTemplateDependencie()){
                        ...
                        //     LOG << k.toString() << " <> " << v.toString() << endln;
                        // }
...
bool TensorExpression::operator==(const TensorExpression& other) const {
    ...
    // LOG << toString(1) << " == " << other.toString(1) << " ???? " << endln;
    ...
    for(size_t childIdx = 0; childIdx < children.size(); childIdx++){
        ...
        if(!(children[childIdx] == other.children[childIdx])){
            ...
            // LOG << children[childIdx].toString() << other.children[childIdx].toString() << endln;
            // LOG << (children[childIdx] == other.children[childIdx]) << endln;
    ...
    //     // LOG << sortedChildren[i].toString() << " == " << sortedOtherChildren[i].toString() << " = " << (sortedChildren[i] == sortedOtherChildren[i]) << endln;
...
void TensorExpression::diffAssign(const TensorExpression& other){
    ...
    for (; it != tensorExpressionDiffTemplates.end(); ++it) {
        ...
        {
            //
            // LOG << "Template found " << it->first.first.toString() << " / " << it->first.second.toString() << " => " << it->second.toString() << endln;
    ...
    if(IsRepresentableByTemplate){
        ...
        // LOG << it->first.first.toString() << " / " << it->first.second.toString() << " => " << it->second.toString() << endln;
...
std::ostream& operator<<(std::ostream& os, const TensorExpression& expr){
    ...
    if(expr.Relation == TkType::Argument){
        ...
        os << "Operation : " << TensorExpressionOperatorStrings[expr.Operator] << " {" << endln;
    ...
    bool TENSOR_EXPRESSION::setUpClass(){
        ...
        registerFunction("logPresetDiffs", {},
            [__functionLabel__ = "logPresetDiffs", __numArgs__ = 0](FREG_ARGS){
                ...
                LOG << "Found " << tensorExpressionDiffs.size() << " registered Diffs :" << endln;

                for(const auto& [k,v] : tensorExpressionDiffs){

                    LOG << "diff[ " << k.first.toString(1) << ", " << k.second.toString(1) << " ] = " << v.toString(1) << endln;
                }
        ...
        registerFunction("logDiffTemplates", {},
            [__functionLabel__ = "logDiffTemplates", __numArgs__ = 0](FREG_ARGS){
                ...
                LOG << "Found " << tensorExpressionDiffTemplates.size() << " registered Diff Templates :" << endln;
        ...
        registerFunction("logSimplifications", {},
            [__functionLabel__ = "logSimplifications", __numArgs__ = 0](FREG_ARGS){
                ...
                LOG << "Found " << tensorExpressionSimplifications.size() << " registered Simplifications :" << endln;

                for(const auto& [k,v] : tensorExpressionSimplifications){

                    LOG << k.toString(1) << " => " << v.toString(1) << endln;
                }
